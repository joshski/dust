# Add Idea: Support docker compose per repo
...
When running `dust bucket worker` or `dust loop`, ideally each iteration would start in a repo-specific docker compose setup, with a named docker “project”, isolated from others.
