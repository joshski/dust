# Decompose Idea: Primitive Obsession Audit

Create one or more well-defined tasks from this idea. Prefer smaller, narrowly scoped tasks that each deliver a thin but complete vertical slice of working software -- a path through the system that can be tested end-to-end -- rather than component-oriented tasks (like "add schema" or "build endpoint") that only work once all tasks are done. Split the idea into multiple tasks if it covers more than one logical change. Run `dust principles` to link relevant principles and `dust facts` for design decisions that should inform the task. See [Primitive Obsession Audit](../ideas/primitive-obsession-audit.md).

Really think about "Functional Core, Imperative Shell"

## Resolved Questions

### Should this audit prioritize literals with existing type definitions?

**Decision:** Option: Prioritize existing-type drift

### Should numeric thresholds be part of scope in v1?

**Decision:** Option: Yes, include numeric magic values


## Decomposes Idea

- [Primitive Obsession Audit](../ideas/primitive-obsession-audit.md)

## Blocked By

(none)

## Definition of Done

- [ ] One or more new tasks are created in .dust/tasks/
- [ ] Task's Principles section links to relevant principles from .dust/principles/
- [ ] The original idea is deleted or updated to reflect remaining scope
