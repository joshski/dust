# Add Idea: Progressive output in `dust check`
...
Instead of dots, make `dust check` output in a deterministic order, (the order the checks are declared) but write the output for each check as soon as that check has finished. Then it's up to users to declare the slowest checks last. But that feedback is better than dots.
