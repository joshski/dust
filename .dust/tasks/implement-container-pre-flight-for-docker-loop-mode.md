...
When running in Docker mode, `runOneIteration` should build a container-aware `ShellRunner` and pass it to `runPreflightChecks`. This ensures install and `dust check` run inside the container, matching the agent's environment.
