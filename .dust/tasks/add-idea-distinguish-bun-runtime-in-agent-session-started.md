# Add Idea: Distinguish bun runtime in agent-session-started
...
When we capture `agent-session-started` events we should capture the `runtime` separately from the `runtimeVersion`. When it's `bun` we should include the Bun.js version in the `runtimeVersion` attribute.

see .dust/facts/dust-event-protocol.md
