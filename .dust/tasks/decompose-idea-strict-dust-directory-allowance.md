# Decompose Idea: Strict .dust directory allowance

Create one or more well-defined tasks from this idea. Prefer smaller, narrowly scoped tasks that each deliver a thin but complete vertical slice of working software -- a path through the system that can be tested end-to-end -- rather than component-oriented tasks (like "add schema" or "build endpoint") that only work once all tasks are done. Split the idea into multiple tasks if it covers more than one logical change. Run `dust principles` to link relevant principles and `dust facts` for design decisions that should inform the task. See [Strict .dust directory allowance](../ideas/strict-dust-directory-allowance.md).

Really think about "Functional Core, Imperative Shell"

## Resolved Questions

### Should `extraDirectories` continue to allow arbitrary directories under `.dust/`?

**Decision:** Option: Deprecate `extraDirectories` for lint allowlisting

### Should `.dust/Dockerfile` be part of the built-in allowlist?

**Decision:** Option: No, require Docker config to move under `.dust/config/`

### How strict should validation be for `.dust/config/` contents?

**Decision:** Option: Approve known files and known subdirectories only


## Decomposes Idea

- [Strict .dust directory allowance](../ideas/strict-dust-directory-allowance.md)

## Blocked By

(none)

## Definition of Done

- [ ] One or more new tasks are created in .dust/tasks/
- [ ] Task's Principles section links to relevant principles from .dust/principles/
- [ ] The original idea is deleted or updated to reflect remaining scope
