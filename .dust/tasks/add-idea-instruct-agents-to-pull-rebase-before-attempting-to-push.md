# Add Idea: Instruct agents to `pull --rebase` before attempting to push
...
When agents push before rebasing, they often end up running all checks once more than necessary (once before the push fails, another time before the push that succeeds). By instructing them to pull (with rebase) before pushing, they can avoid this wasted effort.
