# Add Idea: Proxy servers for docker containers
...
Docker containers started by `dust bucket worker` should not include any secrets - access to the dust bucket server, git, etc - should all happen via the proxy. That way agents can never exfiltrate secrets
