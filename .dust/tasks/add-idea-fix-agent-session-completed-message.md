# Add Idea: Fix “Agent session completed” message
...
At the bottom of a long session (despite knowing it was Claude or codex) we show the message “Agent session completed”. It should show as “Claude session completed” even when there is pagination on the events. Do we need to store the agent type on each session to make this possible?
