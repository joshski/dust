# Add Idea: Interpolation into audits
...
When audits are added using the exported audits package, some audits would benefit from mentioning dust commands. But the runtime detection for which dust command to run (bun/npm/etc) is not necessarily available in the environment that calls the audit API (downstream web app that uses GitHub api). How can we work around this?

(One potential option is to mention in early instructions to the agent that any mention of `dust` should be executed as `bunx dust` or whatever)
