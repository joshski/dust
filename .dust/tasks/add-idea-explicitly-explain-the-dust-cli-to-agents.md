# Add Idea: Explicitly explain the dust cli to agents
...
In all agent prompts (either `dust agent` or provided to the agent in `dust bucket worker` and `dust loop` commands) we should briefly explain what the CLI is and how to run it, and explain that "when you see a suggestion to run `dust ...` that you might need to prefix that with `bunx` or `npx`.
