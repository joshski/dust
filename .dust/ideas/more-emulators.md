...
- **FileSystemEmulator** ([`lib/test/test-utilities.ts`](../../lib/test/test-utilities.ts)) - In-memory file system with read/write tracking
- **ContextEmulator** ([`lib/test/test-utilities.ts`](../../lib/test/test-utilities.ts)) - Captures stdout/stderr for assertions
- **ShellEmulator** ([`system-tests/support/shell-emulator.ts`](../../system-tests/support/shell-emulator.ts)) - Executes CLI commands in-memory
- **AgentEmulator** ([`system-tests/support/agent-emulator.ts`](../../system-tests/support/agent-emulator.ts)) - Pattern-based AI session simulation
- **VCR cassettes** ([`lib/claude/vcr.ts`](../../lib/claude/vcr.ts)) - Record/replay for Claude Code interactions

Despite this, some lower-level patterns remain:

1. **Process stream spying** - `vi.spyOn(process.stdout, 'write')` in [`lib/logging/index.test.ts`](../../lib/logging/index.test.ts)
2. **Global fetch assignment** - `globalThis.fetch = ...` in [`lib/bucket/auth.test.ts`](../../lib/bucket/auth.test.ts)
3. **EventEmitter stubs for spawn** - Manual ChildProcess-like objects in [`lib/bucket/repository.test.ts`](../../lib/bucket/repository.test.ts), [`lib/claude/spawn-claude-code.test.ts`](../../lib/claude/spawn-claude-code.test.ts)
4. **Date.now spying** - `vi.spyOn(Date, 'now')` in [`lib/cli/commands/check.test.ts`](../../lib/cli/commands/check.test.ts)
5. **WebSocket stubs** - Manual WebSocketLike objects in [`lib/bucket/events.test.ts`](../../lib/bucket/events.test.ts)
