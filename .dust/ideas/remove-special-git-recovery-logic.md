...
When `runOneIteration` (in `lib/cli/commands/loop.ts:311-363`) runs, it first attempts a `git pull`. If the pull fails (e.g., merge conflicts, uncommitted local changes), rather than failing immediately, it spawns Claude with a prompt to resolve the issue:
...
- `lib/cli/commands/loop.ts:311-363` — the current git recovery implementation
