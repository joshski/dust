...
Currently, the implementation instructions generated by `buildImplementationInstructions` in `lib/cli/commands/focus.ts:14` tell agents to run `dust check` as their first step. This means every agent session begins with the agent running checks, interpreting the output, and potentially discovering the project is in a bad state.

The `dust loop claude` command (`lib/loop/loop.ts`) and `dust bucket` repository loop (`lib/bucket/repository.ts`) both follow this pattern:
...
- `lib/loop/iteration.ts:98` - `runOneIteration` function that orchestrates each loop iteration
- `lib/bucket/repository-loop.ts:300` - `runRepositoryLoop` function for bucket mode
- `lib/cli/commands/check.ts` - The `check` command implementation
- `lib/cli/commands/focus.ts:14` - `buildImplementationInstructions` which currently tells agents to run checks

The loop already has a pattern for handling pre-task issues: when `git pull` fails, it spawns Claude with a special "resolve git conflict" prompt (`lib/loop/iteration.ts`). The same pattern could be applied for failed checks.
...
In `lib/loop/iteration.ts`, pass `skipPreflightSteps: true` when calling `buildImplementationInstructions`. The `dust focus` command (interactive usage) continues to pass `undefined`, keeping current behavior.
...
**Decision:** Run checks synchronously before task selection.
...
**Decision:** Yes, identical behavior.
