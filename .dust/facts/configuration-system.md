...
Dust uses a JSON configuration file at [`.dust/config/settings.json`](../config/settings.json) to customize behavior.

The [`.dust/config/`](../config) directory is strictly allowlisted by `dust lint`. Valid entries are:
...
Settings are loaded by [`lib/config/settings.ts`](../../lib/config/settings.ts). The `loadSettings` function reads from [`.dust/config/settings.json`](../config/settings.json) and returns defaults if the file doesn't exist. Validation is performed by the exported `validateSettingsJson` function, which validates known keys (dustCommand, checks, installCommand, eventsUrl, and deprecated extraDirectories), reports unknown keys, and flags `extraDirectories` as deprecated.
