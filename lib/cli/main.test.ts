...
describe("runCommand", () => {
  test("runs help command and outputs help text", async () => {
    ...
    expect(ctx.stdoutLines.join("\n")).toContain("dust - A lightweight planning system");
  });
...
describe("main", () => {
  ...
  test("returns error for unknown command", async () => {
    ...
    expect(ctx.stderrLines.join("\n")).toContain("Run 'dust help' for available commands");
  });
  ...
  test("passes command args to subcommands", async () => {
    ...
    expect(result.exitCode).toBe(1);
    expect(ctx.stderrLines.some((line) => line.includes("Invalid type: invalid-type"))).toBe(true);
  });
});
